...
void PowerBufferInit(void)
{
	PBS.max_p = 80.0;
	PBS.max_w = 0.0;//功率单位W
	PBS.r_w = 60.0;//功率单位W
	PBS.r_p = 0.0;//功率单位W
	PBS.current_mapp_coe = 0.00122;//电流映射系数
	PBS.high_water_level = 40.0;
	PBS.low_water_level = 30.3;
	PBS.mid_water_level = 20.8;
	PBS.period = 0.01;//运行周期，单位/s
	PBS.high_current_threshold = 14.0;//A
  PBS.mid_current_threshold = 6.5;//A
  PBS.low_current_threshold = 4.0;//A
  PBS.safe_current_threshold = 2.2;//A
}
/**
  ...
  */
  uint8_t Inject(powerBufferPoolStruct* pbs)
  {
    uint8_t status = 0;
		...
		pbs->r_p = ext_power_heat_data.chassis_power;
		pbs->r_w = ext_power_heat_data.chassis_power_buffer;
